...
func (k msgServer) BuyName(goCtx context.Context, msg *types.MsgBuyName) (*types.MsgBuyNameResponse, error) {
	...
	// Set the price at which name has to be bought if it didn't an owner before
	...
	// Convert owner and buyer address to sdk.AccAddress
	...
	if isFound {
		...
		// Otherwise (when the bid is higher), send tokens from buyer's account to the module's account (as a payment for the name)
